...
/**
 * Doer-side support actions. The Done (customer) app's equivalent
 * (src/app/support/actions.ts there) is the same shape with
 * created_by_role: "requester" and an own-tasks-by-requester_id ownership
 * check — both write to the same shared support_tickets /
 * support_ticket_messages / support_ticket_status_history tables
 ...
 * "I can't sign in", "a Requester was rude", "the app crashed".
 */
export async function createTicket(
  ...
  if (relatedTaskId) {
    const { data: task } = await supabase
      ...
      .eq("doer_id", user.id)
      .maybeSingle();
    if (!task) return { error: "That job couldn't be found on your account." };
  }
...
export async function closeMyTicket(ticketId: string): Promise<{ error?: string }> {
  ...
  try {
    await transitionTicket(supabase, {
      ...
      note: "Confirmed resolved by Doer",
    });
  } catch (e) {
...
export async function reopenMyTicket(ticketId: string, formData: FormData): Promise<{ error?: string }> {
  ...
  try {
    await transitionTicket(supabase, {
      ...
      actor: "doer",
      changedByUser: user.id,
      note: note || "Reopened by Doer — issue not resolved",
    });
  } catch (e) {
    return { error: friendlyError(e) };
  }
